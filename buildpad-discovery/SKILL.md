...
Then create `.kiro/specs/<feature>/brief.md` for **every** feature listed under `## Specs (dependency order)` using the Path C brief format. This enables parallel spec creation via `/buildpad-spec-batch`.
...
Path E rules:
- Keep `## Specs (dependency order)` reserved for **new specs only** so `/buildpad-spec-batch` can still parse it unchanged
...
- Path A: `/buildpad-spec-requirements {feature}` to update the existing spec
- Path B: Recommend direct implementation without creating a spec
- Path C: Default to `/buildpad-spec-init <feature-name>`
  - Optional fast path: `/buildpad-spec-quick <feature-name>` when the user explicitly wants to continue immediately
- Path D: Default to `/buildpad-spec-batch` (creates all specs in parallel based on roadmap.md dependency order)
  - Optional cautious path: `/buildpad-spec-init <first-feature-name>` when the user wants to validate the first slice before batching the rest
- Path E: Choose the next command based on the new-spec portion of the decomposition
  - If there is exactly one new spec: `/buildpad-spec-init <new-feature-name>`
  - If there are multiple new specs: `/buildpad-spec-batch`
  - Also note which existing specs should be revisited with `/buildpad-spec-requirements <feature>`
- Re-entry: `/buildpad-spec-init <next-feature-name>` or `/buildpad-spec-batch` if multiple specs remain
