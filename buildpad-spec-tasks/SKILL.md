...
- If auto-approve flag (`-y`) is true:
  ...
  - Respond: "Tasks generated and auto-approved. Start implementation with `/buildpad-impl {feature}`"
- Otherwise (interactive):
  ...
  - If the user approves:
    - Set `approvals.tasks.approved: true` in spec.json
    - Respond: "Tasks approved. Start implementation with `/buildpad-impl {feature}`"
...
- **Suggested Action**: "Run `/buildpad-spec-tasks {feature} -y` to auto-approve all (requirements, design, and tasks) and proceed"
...
- **Suggested Action**: "Refine requirements.md or design.md, then re-run `/buildpad-spec-tasks {feature}`"
...
- Autonomous implementation: `/buildpad-impl {feature}`
- Specific tasks only: `/buildpad-impl {feature} 1.1,1.2`
