...
## Execution Steps
1. **Check for Brief**: If `.kiro/specs/{feature-name}/brief.md` exists (created by `/buildpad-discovery`), read it. The brief contains problem, approach, scope, and constraints from the discovery session. Use this to pre-fill the project description and skip clarification questions that the brief already answers.
...
5. **Initialize Files Using Templates**:
   - If `.kiro/settings/templates/specs/` does not exist, bootstrap it first: copy this skill's bundled `settings-templates/specs/` directory to `.kiro/settings/templates/specs/`
...
4. **Next Step**: Command block showing `/buildpad-spec-requirements <feature-name>`
