...
1. **Check for Brief**:
   - If `.kiro/specs/{feature-name}/brief.md` exists (created by `/buildpad-discovery`), read it for discovery context (problem, approach, scope, constraints)
...
Invoke `/buildpad-spec-requirements {feature-name}` via the Skill tool.
...
Invoke `/buildpad-spec-design {feature-name} -y` via the Skill tool. The `-y` flag auto-approves requirements.
...
Invoke `/buildpad-spec-tasks {feature-name} -y` via the Skill tool.
...
Skipped: /buildpad-validate-gap, /buildpad-validate-design

Sanity review: PASSED | FOLLOW-UP REQUIRED

Next Steps:
1. Review generated specs (especially design.md)
2. Optional: `/buildpad-validate-gap {feature}`, `/buildpad-validate-design {feature}`
3. Start implementation: `/buildpad-impl {feature}`
```
...
- Suggest: "Continue manually from `/buildpad-spec-{next-phase} {feature}`"

**Sanity Review Failed**:
- Stop workflow
- Report the exact contradiction, missing prerequisite, or task-plan issue
- Suggest targeted follow-up with `/buildpad-spec-design {feature}`, `/buildpad-spec-tasks {feature}`, or manual edits depending on the finding
