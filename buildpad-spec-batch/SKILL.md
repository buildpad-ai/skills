...
5. If any brief.md is missing, stop and report: "Missing brief.md for: [list]. Run `/buildpad-discovery` to generate briefs first."
...
3. Execute the full spec pipeline. For each phase, read the corresponding skill's SKILL.md for complete instructions (templates, rules, review gates):
   a. Initialize: Read .claude/skills/buildpad-spec-init/SKILL.md, then create spec.json and requirements.md
   b. Generate requirements: Read .claude/skills/buildpad-spec-requirements/SKILL.md, then follow its steps
   c. Generate design: Read .claude/skills/buildpad-spec-design/SKILL.md, then follow its steps
   d. Generate tasks: Read .claude/skills/buildpad-spec-tasks/SKILL.md, then follow its steps
...
Next: Review generated specs, then start implementation with /buildpad-impl <feature>
```
...
- Suggest: "Run `/buildpad-spec-quick <feature> --auto` manually for failed features."
...
**Roadmap not found**:
- Stop and report: "No roadmap.md found. Run `/buildpad-discovery` first."
