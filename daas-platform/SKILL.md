...
| Third-party API calls (Stripe, etc.) | Connect via the Buildpad platform's Connectors page; credential(s) arrive as whitelisted env var(s) — check `get_project_detail`'s `connectors[]` for the exact name(s), base URL, and auth header style (base URL may itself be one of the env vars for a self-hosted provider like Chocolate Factory). Most providers are called from a custom service via raw `services.fetch`, but check that provider's `references/<key>.instructions.md` first — one that ships an official SDK (e.g. Chocolate Factory) is called from a server-side Next.js API route instead, since custom services can't import npm packages. For local dev, `connectors[]` also carries `envVars` (actual decrypted values) to populate `.env.local` — and since connecting a provider does NOT push anything to the deployed Amplify app, also push that same `envVars` map with `amplify_set_env_vars` + `amplify_redeploy` (see `amplify-env-vars` skill) once the feature is built | Hardcoding API keys, inventing your own env var name, asking the user to paste a key into code, building a connector-dependent feature without pushing its env var(s) to Amplify (works locally, silently missing once deployed) |
