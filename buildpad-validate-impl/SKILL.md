...
Per-task checks are the reviewer's responsibility during `/buildpad-impl`. This skill does **not** re-check:
...
- Parse conversation history for `/buildpad-impl` commands to detect recently implemented features and tasks
...
Before returning `GO`, apply the `buildpad-verify-completion` protocol to the feature-level claim. Tests alone are insufficient: include full-suite, runtime liveness, coverage, integration, design-alignment, and blocked-task status in the evidence.
...
- Re-run `/buildpad-impl {feature} [tasks]` for targeted fixes
- Re-validate with `/buildpad-validate-impl {feature}`
